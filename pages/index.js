...
export default function Home() {
  ...
  useEffect(() => {
    ...
    async function getUserUid() {
      ...
      const posts = friendId.map(
        async (uid) => await getDocs(collection(db, "users", uid, "posts"))
      );
      const allPost = posts.map((doc) =>
        doc.then((doc) =>
          doc.docs.map((doc) => ({ ...doc.data(), id: doc.id }))
        )
      );
      const post = await Promise.all([
        ...allPost.map((doc) => doc.then((response) => response)),
      ]);
  ...
  return (
    <div className="container mx-auto">
      <div className="flex items-center justify-center h-full w-full">
        ...
        <HomeContactFriendsList
          setContactModal={setContactModal}
          openContactModal={openContactModal}
        />
        {openContactModal && (
          <ContactHomeFriendsListModal
            setContactModal={setContactModal}
            openContactModal={openContactModal}
          />
        )}
