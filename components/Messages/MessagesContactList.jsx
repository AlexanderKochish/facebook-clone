    ...
    useEffect(()=>{
        ...
        const getContactUsers = async() => {
          try {
            ...
            setContactsFriends(res.docs.map((doc)=>({...doc.data(),id:doc.id}))) 
          } catch (error) {
            console.log(error)
          }  
    }
...
console.log(state);
    const getChatUser = async(user) => {
      ...
      const combaindId = currentUser.uid > user.uid? currentUser?.uid + user.uid : user.uid + currentUser?.uid;
    ...
    <div className='w-1/4 min-h-screen flex flex-col pt-16 px-4 bg-white'>
      <div className='mb-2 min-h-[120px] w-full flex flex-col'>
        ...
        <ul className='flex items-center space-x-2'>
            <li className='p-2 rounded-[50%] bg-gray-200 hover:bg-gray-300 cursor-pointer'><HiOutlineDotsHorizontal className='w-6 h-6'/></li>
            <li className='p-2 rounded-[50%] bg-gray-200 hover:bg-gray-300 cursor-pointer'><RiVideoAddFill className='w-6 h-6'/></li>
            <li className='p-2 rounded-[50%] bg-gray-200 hover:bg-gray-300 cursor-pointer'><BsPencilSquare className='w-6 h-6'/></li>
        </ul>
        </div>
        <div className='bg-gray-100 rounded-2xl mt-2  flex items-center w-full px-3'>
          <BiSearchAlt2 className='w-6 h-6 text-slate-500'/>
          <input type="text" placeholder='Search in messenger' className='w-full px-3 outline-none py-2 bg-inherit'/>
        </div>
        </div>
        <ul>
        
          {contactsFriends.map(({user})=>(
        <li key={user.uid} onClick={()=>getChatUser(user)}  className='flex items-center justify-between hover:bg-slate-100 rounded-md px-2 py-3 cursor-pointer'>
