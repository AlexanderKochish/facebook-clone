    ...
    <div className='fixed top-0 left-0 right-0 w-screen min-h-full flex items-center justify-center z-30 bg-white/80'>
        <div className='bg-white min-w-[500px] p-5 min-h-[400px] rounded-lg drop-shadow-2xl absolute z-40'>
            <div className='flex items-center justify-between border-b p-3'>
                ...
                <button onClick={showModalProfilePhoto} className='bg-gray-200 hover:bg-slate-300 rounded-[50%] p-2'>
            ...
            <ul className='flex flex-col w-full py-5'>
                <li className='flex justify-center bg-slate-200 hover:bg-slate-300 w-full py-2 rounded-lg'>
                    <input type='file' className='hidden' onChange={(e)=>setFile(e.target.files[0])} id='upload_profile_photo'/>
                    <label htmlFor='upload_profile_photo' className='w-full h-full flex cursor-pointer justify-center items-center'>
            ...
            <div>
                ...
                {profileGallery?.map(({id,image})=>(
                    <li key={id} className='flex self-start'>
                        <div onClick={()=>handleSelectProfilePhoto(image)} className='relative w-36 h-40 m-1 cursor-pointer hover:scale-105 duration-300'>
                            <Image src={image} fill alt='profile' className='object-cover rounded-md'/>
                        </div>
                    </li>
                ))}
                </ul>
