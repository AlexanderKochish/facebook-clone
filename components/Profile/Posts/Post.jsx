...
const Post = ({post,profile}) => {
  ...
  return (
    <>
    {post && (
    <div key={post.id} className='w-full min-h-[100px] bg-white rounded-md drop-shadow-md flex flex-col py-3 mt-4'>
        <div className='flex items-center justify-between mb-2 px-3'>
            <div className='flex space-x-2'>
                <div className='relative w-12 h-12'>
                    <Image src={profile?.photoURL || userPhoto} fill alt='currentUser' className='object-cover rounded-[50%]'/>
                </div>
                <div>
        ...
        {post.image? <div className='relative w-full h-96'>
            <Image src={post?.image || userPhoto} fill alt='post content' className='object-cover object-center'/>
        </div>:''}
        ...
        <div className='flex flex-col my-2'>
            ...
            <div className='w-full flex items-center mt-2 px-3'>
                ...
                <Image src={profile?.photoURL || userPhoto} alt='profile' fill className='object-cover rounded-[50%]'/>
                </div>
                <form className='w-full rounded-2xl p-1 bg-gray-200'>
                    <input type="text" placeholder='write commets' className='outline-none bg-inherit px-3 py-2 w-full'/>
                </form>
            </div>
        </div>
    </div>
