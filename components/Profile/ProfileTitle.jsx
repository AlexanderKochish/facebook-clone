...
const ProfileTitle = ({profile}) => { 
    ...
    useEffect(()=>{
        ...
        const getBgImage = async() => {
            const currentUserGalleryRef = doc(db, 'users', profile?.uid, 'gallery', 'coverPhoto')
  ...
  return (
    <div className="flex flex-col self-start w-full min-h-[680px] shadow-md bg-gradient-to-b from-[#A8CAE5] via-white to-white">
        <div className="w-[1100px] mx-auto">
            <div className="relative w-full h-[450px]">
                <Image src={currentTitleBgImage?.image || userPhoto} fill alt="profile" className="object-cover rounded-b-xl"/>
               {currentUser?.uid !== profile?.uid? '': <div onClick={()=>setBgImage(!bgImage)} className="absolute z-10 bottom-5 right-10 cursor-pointer items-center py-2 bg-slate-100 rounded-lg flex space-x-3 px-2">
            ...
            <div className="flex items-center justify-between px-10 border-b-2 pb-5">
                ...
                <div className="relative -mt-10 w-48 h-48 border-white rounded-[50%] border-4 mr-5">
                    ...
                    {currentUser?.uid !== profile?.uid? '':<div onClick={showModalProfilePhoto} className="p-2 bg-gray-200 hover:bg-gray-300 rounded-[50%] absolute bottom-0 right-5 cursor-pointer">
                        <MdPhotoCamera className="w-5 h-5"/>
                    </div>}
                </div>
               {!uploadProfilePhoto && <Modal profile={currentUser} uploadProfilePhoto={uploadProfilePhoto} setProfilePhoto={setProfilePhoto}/>}
                ...
                <ul className="flex items-center text-lg text-slate-500 font-semibold">
                    <Link href={`/profile/${router.query.profileUid}`}>
                        <li className={router.pathname === '/profile/[profileUid]'?
                        'px-4 py-3 text-sky-600 cursor-pointer border-b-4 border-sky-600'
                        :
                        'px-4 py-2 hover:bg-slate-200 rounded-md cursor-pointer'}>Publications</li>
                    </Link>
                    <li className="px-4 py-2 hover:bg-slate-200 rounded-md cursor-pointer">Info</li>
                    <li className="px-4 py-2 hover:bg-slate-200 rounded-md cursor-pointer">Friends</li>
                    <Link href={`/profile/${router.query.profileUid}/photos`}>
                        <li className={router.pathname === '/profile/[profileUid]/photos'?
                        'px-4 py-3 text-sky-600 cursor-pointer border-b-4 border-sky-600'
                        :
                        'px-4 py-2 hover:bg-slate-200 rounded-md cursor-pointer'
                        }>Photo</li>
                    </Link>
                    <li className="px-4 py-2 hover:bg-slate-200 rounded-md cursor-pointer">Video</li>
                    <li className="px-4 py-2 hover:bg-slate-200 rounded-md cursor-pointer">position</li>
                </ul>
                <div className="px-4 py-2 bg-slate-200 rounded-md hover:bg-slate-300 cursor-pointer text-lg text-slate-700 font-semibold my-2">
